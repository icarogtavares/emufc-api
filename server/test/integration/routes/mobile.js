...
const {
  version: Version, responsible: Responsible, place: Place, equipment: Equipment,
} = db
...
describe('Routes: Equipment', () => {
  ...
  before((done) => {
    Equipment.drop()
      .then(() => Promise.all([Version.sync({ force: true }), Responsible.sync({ force: true }), Place.sync({ force: true })]))
      .then(() => Promise.all([Version.create(fakeVersion), Place.bulkCreate(fakePlaces), Responsible.bulkCreate(fakeResponsibles)]))
      .then(() => Equipment.sync({ force: true }))
