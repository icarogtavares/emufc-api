...
describe('Routes: Equipment', () => {
    ...
    const syncPlaces = Place.sync({ force: true }).then(() => Place.create(fakePlace));

    const syncResponsibles = Responsible.sync({ force: true }).then(() => Responsible.create(fakeResponsible));

    before(done => {
        Promise.all([syncPlaces, syncResponsibles]).then(() => {
            done();
        }).catch(err => done(err));
    });
